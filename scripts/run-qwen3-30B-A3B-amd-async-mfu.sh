#!/bin/bash
#
# MFU-optimized variant of run-qwen3-30B-A3B-amd-async.sh
# Differences from Run B baseline:
#   1. --attention-backend fused          (use TE 2.12 CK fused attn / aiter ASM kernels)
#   2. recompute disabled                  (3 args removed -> visible MFU win)
#   3. --overlap-grad-reduce              (DP gradient reduce-scatter overlapped with bwd)
#   4. --overlap-param-gather             (distributed-optimizer param all-gather overlapped with fwd)
#   5. NVTE_FLASH_ATTN=0/NVTE_FUSED_ATTN=1/NVTE_UNFUSED_ATTN=0 in runtime env
#      (overrides the container's baked-in NVTE_FUSED_ATTN=0; required for --attention-backend fused)
#   6. wandb-group qwen3-30B-A3B-mfu       (separate from Run B "qwen3-30B-A3B-test")
