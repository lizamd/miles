...
# Recipe: hybrid format (E4M3 fwd, E5M2 bwd) + Float8CurrentScaling (tensorwise).
# No amax history -> no warmup window like delayed scaling has.
# SGLang rollout also FP8 (--sglang-quantization fp8) so train and rollout use
# the same precision -> minimizes train_rollout_logprob_abs_diff.
# wandb-group qwen3-30B-A3B-fp8-tensorwise
...
SGLANG_ARGS=(
   ...
   --sglang-dp-size 2
   --sglang-ep-size 2
   # NOTE: tried --sglang-quantization fp8 but miles weight-update path doesn't
   # transpose weights for FP8 SGLang layout -> first update_weights_from_distributed
   # fails with shape mismatch [2048,4096] vs [4096,2048]. Keeping SGLang in bf16.
)
...
MISC_ARGS=(
   ...
   --moe-grouped-gemm
   # Memory: hard cap per-expert tokens. NOTE: --moe-pad-expert-input-to-capacity
   # caused NCCL allreduce hang in dispatcher (timed out after 600s, ranks disagreed
   # on padded shape) -- removed.
